...
post_decontamination::post_decontamination(QSqlDatabase database, QObject *parent) : QObject(parent), db(database) {
    stdin_notifier = nullptr;
}

void post_decontamination::start_barcode_scanning() {
    if (stdin_notifier) {
        return;
    }
    ...
    connect(
        ...
        this,
        &post_decontamination::read_terminal_input
    );

    qDebug() << "Paste barcode into terminal and press Enter.";
}

void post_decontamination::stop_barcode_scanning() {
    if (stdin_notifier) {
...
void post_decontamination::read_terminal_input() {
...
QString post_decontamination::batch_code() const {
    return m_batch_code;
}

QString post_decontamination::type_of_surgery() const {
    return m_type_of_surgery;
}

QString post_decontamination::patients_name() const {
    return m_patients_name;
}

QString post_decontamination::last_scan_date_n_time() const {
    return m_last_scan_date_n_time;
}

void post_decontamination::process_barcode(const QString &barcode) {
...
bool post_decontamination::fetch_batch(const QString &barcode) {
    if (!db.isOpen()) {
    ...
    batch_query.prepare("SELECT created_batches.batch_code, created_batches.type_of_surgery, surgeries.patients_name, surgeries.pre_decontamination_scanned_timestamp FROM created_batches INNER JOIN surgeries ON created_batches.batch_code = surgeries.batch_code WHERE created_batches.batch_code = :barcode AND surgeries.cycle_complete = :complete");
    ...
    m_last_scan_date_n_time = batch_query.value("pre_decontamination_scanned_timestamp").toString();
    ...
    check_query.prepare("SELECT created_batches.post_decontamination, surgeries.cycle_complete FROM created_batches INNER JOIN surgeries ON created_batches.batch_code = surgeries.batch_code WHERE created_batches.batch_code = :barcode AND surgeries.patients_name = :patient");
    ...
    bool post_decontamination_status = check_query.value("post_decontamination").toBool();
    bool cycle_complete_status = check_query.value("cycle_complete").toBool();

    //Ensure users do not skip the previous stage
    if (!post_decontamination_status && !cycle_complete_status) {
        QSqlQuery stage_query(db);

        stage_query.prepare("SELECT pre_decontamination FROM created_batches WHERE batch_code = :barcode");
        ...
        bool pre_decontamination_status = stage_query.value("pre_decontamination").toBool();

        if (!pre_decontamination_status) {
            emit warning_message("The pre_decontamination stage has not been completed. Proceeding to this stage is not permitted.");
            return false;
        }
    ...
    } else if (post_decontamination_status && !cycle_complete_status) {
        emit warning_message("Scanning at post_decontamination stage has already occured. Please proceed to the next stage.");
...
QVariantList post_decontamination::fetch_batch_items(const QString &batch_code) {
    QVariantList items;
...
void post_decontamination::auto_fill() {
...
QString post_decontamination::session_user() const {
    return m_session_user;
}

QString post_decontamination::current_scan_date_n_time() const {
    return m_current_scan_date_n_time;
}

void post_decontamination::confirm_batch(const QString &nurse_name, const QString &scanned_by, const QString &current_scan_date_n_time) {
    ...
    query.prepare("UPDATE surgeries SET post_decontamination_nurse_name = :nurse_name, post_decontamination_scanned_by = :scanned_by, post_decontamination_scanned_timestamp = :timestamp WHERE batch_code = :barcode AND patients_name = :patient");
    ...
    if (!query.exec()) {
        ...
        emit warning_message("Failed to save the post decontamination record." + query.lastError().text());
        return;
    }

    QSqlQuery stage_update_query(db);

    stage_update_query.prepare("UPDATE created_batches SET post_decontamination = :decontamination WHERE batch_code = :barcode");

    stage_update_query.bindValue(":barcode", m_batch_code);
    stage_update_query.bindValue(":decontamination", true);
...
void post_decontamination::reject_batch(const QString &nurse_name, const QString &comment) {
    QString rejection_stage = "post_decontamination";
