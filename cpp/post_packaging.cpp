...
post_packaging::post_packaging(QSqlDatabase database, QObject *parent) : QObject(parent), db(database) {
    stdin_notifier = nullptr;
}

void post_packaging::start_barcode_scanning() {
    if (stdin_notifier) {
        return;
    }
    ...
    connect(
        ...
        this,
        &post_packaging::read_terminal_input
    );

    qDebug() << "Paste barcode into terminal and press Enter.";
}

void post_packaging::stop_barcode_scanning() {
    if (stdin_notifier) {
...
void post_packaging::read_terminal_input() {
...
QString post_packaging::batch_code() const {
    return m_batch_code;
}

QString post_packaging::type_of_surgery() const {
    return m_type_of_surgery;
}

QString post_packaging::patients_name() const {
    return m_patients_name;
}

QString post_packaging::last_scan_date_n_time() const {
    return m_last_scan_date_n_time;
}

void post_packaging::process_barcode(const QString &barcode) {
...
bool post_packaging::fetch_batch(const QString &barcode) {
    if (!db.isOpen()) {
    ...
    batch_query.prepare("SELECT created_batches.batch_code, created_batches.type_of_surgery, surgeries.patients_name, surgeries.pre_packaging_scanned_timestamp FROM created_batches INNER JOIN surgeries ON created_batches.batch_code = surgeries.batch_code WHERE created_batches.batch_code = :barcode AND surgeries.cycle_complete = :complete");
    ...
    m_last_scan_date_n_time = batch_query.value("pre_packaging_scanned_timestamp").toString();

    QSqlQuery check_query(db);

    check_query.prepare("SELECT created_batches.post_packaging, surgeries.cycle_complete FROM created_batches INNER JOIN surgeries ON created_batches.batch_code = surgeries.batch_code WHERE created_batches.batch_code = :barcode AND surgeries.patients_name = :patient");
    ...
    bool post_packaging_status = check_query.value("post_packaging").toBool();
    bool cycle_complete_status = check_query.value("cycle_complete").toBool();

    if (!post_packaging_status && !cycle_complete_status) {
        QSqlQuery stage_query(db);

        stage_query.prepare("SELECT pre_packaging FROM created_batches WHERE batch_code = :barcode");
        ...
        bool pre_theatre_status = stage_query.value("pre_packaging").toBool();

        if (!pre_theatre_status) {
            emit warning_message("The pre packaging stage has not been completed. Proceeding to this stage is not permitted.");
            return false;
        }
    ...
    } else if (post_packaging_status && !cycle_complete_status) {
        emit warning_message("Scanning at post packaging stage has already occured. Please proceed to the next stage.");
...
QVariantList post_packaging::fetch_batch_items(const QString &batch_code) {
    QVariantList items;
...
QString post_packaging::session_user() const {
    return m_session_user;
}

QString post_packaging::current_scan_date_n_time() const {
    return m_current_scan_date_n_time;
}

void post_packaging::confirm_batch(const QString &nurse_name, const QString &scanned_by, const QString &current_scan_date_n_time) {
    ...
    query.prepare("UPDATE surgeries SET post_packaging_nurse_name = :nurse_name, post_packaging_scanned_by = :scanned_by, post_packaging_scanned_timestamp = :timestamp WHERE batch_code = :barcode AND patients_name = :patient");
    ...
    if (!query.exec()) {
        ...
        emit warning_message("Failed to save the post packaging record." + query.lastError().text());
        return;
    }

    QSqlQuery stage_update_query(db);

    stage_update_query.prepare("UPDATE created_batches SET post_packaging = :theatre WHERE batch_code = :barcode");
...
void post_packaging::reject_batch(const QString &nurse_name, const QString &comment) {
    QString rejection_stage = "post_packaging";
