...
post_theatre::post_theatre(QSqlDatabase database, QObject *parent) : QObject(parent), db(database) {
    stdin_notifier = nullptr;
}

void post_theatre::start_barcode_scanning() {
    if (stdin_notifier) {
        return;
    }
    ...
    connect(
        ...
        this,
        &post_theatre::read_terminal_input
    );

    qDebug() << "Paste barcode into terminal and press Enter.";
}

void post_theatre::stop_barcode_scanning() {
    if (stdin_notifier) {
...
void post_theatre::read_terminal_input() {
...
QString post_theatre::batch_code() const {
    return m_batch_code;
}

QString post_theatre::type_of_surgery() const {
    return m_type_of_surgery;
}

QString post_theatre::patients_name() const {
    return m_patients_name;
}

QString post_theatre::last_scan_date_n_time() const {
    return m_last_scan_date_n_time;
}

void post_theatre::process_barcode(const QString &barcode) {
...
bool post_theatre::fetch_batch(const QString &barcode) {
    if (!db.isOpen()) {
    ...
    batch_query.prepare("SELECT created_batches.batch_code, created_batches.type_of_surgery, surgeries.patients_name, surgeries.pre_theatre_scanned_timestamp FROM created_batches INNER JOIN surgeries ON created_batches.batch_code = surgeries.batch_code WHERE created_batches.batch_code = :barcode AND surgeries.cycle_complete = :complete");
    ...
    m_last_scan_date_n_time = batch_query.value("pre_theatre_scanned_timestamp").toString();

    QSqlQuery check_query(db);

    check_query.prepare("SELECT created_batches.post_theatre, surgeries.cycle_complete FROM created_batches INNER JOIN surgeries ON created_batches.batch_code = surgeries.batch_code WHERE created_batches.batch_code = :barcode AND surgeries.patients_name = :patient");
    ...
    bool post_theatre_status = check_query.value("post_theatre").toBool();
    bool cycle_complete_status = check_query.value("cycle_complete").toBool();

    if (!post_theatre_status && !cycle_complete_status) {
        QSqlQuery stage_query(db);

        stage_query.prepare("SELECT pre_theatre FROM created_batches WHERE batch_code = :barcode");
        ...
        bool pre_theatre_status = stage_query.value("pre_theatre").toBool();

        if (!pre_theatre_status) {
            emit warning_message("The Pre-Theatre stage has not been completed. Proceeding to this stage is not permitted.");
            return false;
        }
    ...
    } else if (post_theatre_status && !cycle_complete_status) {
        emit warning_message("Scanning at post theatre stage has already occured. Please proceed to the next stage.");
...
QVariantList post_theatre::fetch_batch_items(const QString &batch_code) {
    QVariantList items;
...
QString post_theatre::session_user() const {
    return m_session_user;
}

QString post_theatre::current_scan_date_n_time() const {
    return m_current_scan_date_n_time;
}

void post_theatre::confirm_batch(const QString &nurse_name, const QString &scanned_by, const QString &current_scan_date_n_time) {
    ...
    query.prepare("UPDATE surgeries SET post_theatre_nurse_name = :nurse_name, post_theatre_scanned_by = :scanned_by, post_theatre_scanned_timestamp = :timestamp WHERE batch_code = :barcode AND patients_name = :patient");
    ...
    stage_update_query.prepare("UPDATE created_batches SET post_theatre = :theatre WHERE batch_code = :barcode");
...
void post_theatre::reject_batch(const QString &nurse_name, const QString &comment) {
    QString rejection_stage = "post_theatre";
