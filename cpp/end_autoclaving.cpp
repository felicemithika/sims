...
end_autoclaving::end_autoclaving(QSqlDatabase database, QObject *parent) : QObject(parent), db(database) {
    stdin_notifier = nullptr;
}

void end_autoclaving::start_barcode_scanning() {
    if (stdin_notifier) {
        return;
    }
    ...
    connect(
        ...
        this,
        &end_autoclaving::read_terminal_input
    );

    qDebug() << "Paste barcode into terminal and press Enter.";
}

void end_autoclaving::stop_barcode_scanning() {
    if (stdin_notifier) {
...
void end_autoclaving::read_terminal_input() {
...
QString end_autoclaving::batch_code() const {
    return m_batch_code;
}

QString end_autoclaving::type_of_surgery() const {
    return m_type_of_surgery;
}

QString end_autoclaving::patients_name() const {
    return m_patients_name;
}

QString end_autoclaving::last_scan_date_n_time() const {
    return m_last_scan_date_n_time;
}

void end_autoclaving::process_barcode(const QString &barcode) {
...
bool end_autoclaving::fetch_batch(const QString &barcode) {
    if (!db.isOpen()) {
    ...
    batch_query.prepare("SELECT created_batches.batch_code, created_batches.type_of_surgery, surgeries.patients_name, surgeries.post_packaging_scanned_timestamp FROM created_batches INNER JOIN surgeries ON created_batches.batch_code = surgeries.batch_code WHERE created_batches.batch_code = :barcode AND surgeries.cycle_complete = :complete");
    ...
    m_last_scan_date_n_time = batch_query.value("post_packaging_scanned_timestamp").toString();
    ...
    check_query.prepare("SELECT created_batches.end_autoclaving, surgeries.cycle_complete FROM created_batches INNER JOIN surgeries ON created_batches.batch_code = surgeries.batch_code WHERE created_batches.batch_code = :barcode AND surgeries.patients_name = :patient");
    ...
    bool end_autoclaving_status = check_query.value("end_autoclaving").toBool();
    bool cycle_complete_status = check_query.value("cycle_complete").toBool();

    //Ensure users do not skip the previous stage
    if (!end_autoclaving_status && !cycle_complete_status) {
        QSqlQuery stage_query(db);

        stage_query.prepare("SELECT post_packaging FROM created_batches WHERE batch_code = :barcode");
        ...
        bool post_packaging_status = stage_query.value("post_packaging").toBool();

        if (!post_packaging_status) {
            emit warning_message("The post_packaging stage has not been completed. Proceeding to this stage is not permitted.");
            return false;
        }
    ...
    } else if (end_autoclaving_status && !cycle_complete_status) {
        emit warning_message("Scanning at end_autoclaving stage has already occured. Please proceed to the next stage.");
...
QVariantList end_autoclaving::fetch_batch_items(const QString &batch_code) {
    QVariantList items;
...
void end_autoclaving::auto_fill() {
...
QString end_autoclaving::session_user() const {
    return m_session_user;
}

QString end_autoclaving::current_scan_date_n_time() const {
    return m_current_scan_date_n_time;
}

void end_autoclaving::confirm_batch(const QString &nurse_name, const QString &scanned_by, const QString &current_scan_date_n_time) {
    ...
    query.prepare("UPDATE surgeries SET end_autoclaving_nurse_name = :nurse_name, end_autoclaving_scanned_by = :scanned_by, end_autoclaving_scanned_timestamp = :timestamp WHERE batch_code = :barcode AND patients_name = :patient");
    ...
    if (!query.exec()) {
        ...
        emit warning_message("Failed to save the end autoclaving record." + query.lastError().text());
        return;
    }

    QSqlQuery stage_update_query(db);

    stage_update_query.prepare("UPDATE created_batches SET end_autoclaving = :autoclaving WHERE batch_code = :barcode");

    stage_update_query.bindValue(":barcode", m_batch_code);
    stage_update_query.bindValue(":autoclaving", true);
...
void end_autoclaving::reject_batch(const QString &nurse_name, const QString &comment) {
    QString rejection_stage = "end_autoclaving";
